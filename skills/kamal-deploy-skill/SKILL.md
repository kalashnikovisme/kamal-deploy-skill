...
2. **Docker registry** — ask which option the user wants:
   ...
   - **Local Kamal registry** — a private registry running as a Kamal accessory on the server itself; no external registry account needed

   If the user chooses **local Kamal registry**, apply these modifications to the `deploy.yml` generated by the stack recipe:
   ...
   # accessories: add the registry accessory
   accessories:
     registry:
       image: registry:2
       host: <SERVER_IP>
       port: "127.0.0.1:4443:5000"
       directories:
         - registry-data:/var/lib/registry
   ```

   Also remove `KAMAL_REGISTRY_PASSWORD` from `.kamal/secrets` — it is not needed.

   On first deploy, boot the registry accessory before running `kamal setup`:
   ```bash
